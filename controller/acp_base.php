...
class acp_base
{
	...
	public function get_data()
	{
		return [
			...
			'page_title' => $this->language->lang('ACP_MAS_TITLE') . ' - ' . $this->language->lang('ACP_MAS_' . strtoupper($this->mode)),
		];
	}
	...
	protected function success_form_on_submit()
	{
		...
		$this->log->add('admin', $this->user->data['user_id'], $this->user->ip, 'ACP_MAS_LOG_SET_SAV', time(), array($this->language->lang('ACP_MAS_' . strtoupper($this->mode))));

		// Option settings have been updated and logged
		// Confirm this to the user and provide link back to previous page
		trigger_error($this->language->lang('ACP_MAS_LOG_SET_SAV', $this->language->lang('ACP_MAS_' . strtoupper($this->mode))) . adm_back_link($this->u_action), E_USER_NOTICE);
	}
}
